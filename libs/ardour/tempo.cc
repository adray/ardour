...
/** Returns the  frame corresponding to the initial tempo section in the map */
framepos_t
		    ...
		    framepos_t lower, framepos_t upper, uint32_t bar_mod)
{
	...
	if (bar_mod == 0) {
		while (pos >= 0 && pos < upper) {
			...
			points.push_back (BBTPoint
					  (meter, tempo_at_minute_locked (_metrics, minute_at_frame (pos)), pos, bbt.bars, bbt.beats, qn));
			++cnt;
		}
	} else {
